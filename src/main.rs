...
fn locate_graphic_protocol<'a>(
    ...
    if status != EfiStatus::Success {
        return Err("Failed to locate graphics output protoco");
    }
...
fn draw_test_pattern<T: Bitmap>(buf: &mut T) {
    ...
    draw_str_fg(buf, left, h * colors.len() as i64, 0xff00, "0123456789");
    draw_str_fg(buf, left, h * colors.len() as i64 + 16, 0xff00, "ABCDEF");
}
...
trait Bitmap {
    ...
    /// which means that passing is_in_*_range tets.
